...
while True:
    ...
    if count>25 and flag_capture==1 and flag>1 :
          ...
          if var[1]-var[0]>700 :
            if pts1[-1][1]<pts1[0][1]:
              ...
              #gg=cv2.imread('local.jpeg')
              #cv2.imshow('sent image',gg)
              #cv2.waitKey(0)
    ...
    if center1==None and flag==20:
        print len(pts2)
        var=np.var(np.asarray(pts2),axis=0)
        print var
        if var[1]-var[0]>700 :
            if pts2[-1][1]<pts2[0][1]:
              #print "positive y axis"
              print "LED ON"
